...
var getMovieData = function() {
  ...
  $(".movie-title").html(    "You are interested in finding fans of <strong>" +
    movie +
    "</strong>.");

  URL = base_url + "summary?movie=" + movie.toLowerCase();
  
        $(".movie-summary").html("");

  
  summary_text = "If you start your marketing plan with the recommendations below, you have a much better chance <strong>based on data</strong> of finding them and converting them.<br>";
  fetch(URL)
    .then(response => {
    ...
    .then(data => {
      ...
      summary_text +=
        "FanFinder recommends you <strong>target</strong> <strong style='color: #B07874;'>" +
        summary["demo"]["age"] +
        "</strong>, <strong style='color: #B07874;'>" +
        summary["demo"]["gender"] +
        "</strong>, <strong style='color: #B07874;'>" +
        summary["demo"]["ed"] +
        "</strong>, and <strong style='color: #B07874;'>" +
        summary["demo"]["income"] +
        "</strong> of disposable income per month. <br>";

      summary_text +=
        "Attitudinally, these potential fans agree with the statements <strong style='color: #4A946C;'>" +
        summary["single_attitudes"]["top"][0] +
        "</strong>, and <strong style='color: #4A946C;'>" +
        summary["single_attitudes"]["top"][1] +
        "</strong>.<br> They are fans of <strong>other movies like</strong> <strong style='color: #4A946C;'>" +
        summary["similar_movies"][0] +
        "</strong> & <strong style='color: #4A946C;'>" +
        summary["similar_movies"][1] +
        "</strong>. <br>";

      summary_text +=
        "From a geographic point of view, make sure you focus on <strong style='color: #91A25F;'>" +
        summary["geo"][0] +
        "</strong>, <strong style='color: #91A25F;'>" +
        summary["geo"][1] +
        "</strong>, and <strong style='color: #91A25F;'>" +
        summary["geo"][2] +
        "</strong> " +
        ". These regions have the highest concentration of the types of fans you are looking for.<br>";

      summary_text +=
        "From a <strong>marketing</strong> point of view, FanFinder recommends you have an <strong>social media</strong> approach that has a presence on <strong style='color: #9FAECB;'>" +
        summary["socmed"][0] +
        "</strong> and <strong style='color: #9FAECB;'>" +
        summary["socmed"][1] +
        "</strong>. " +
        "The recommended <strong>advertising channels</strong> are <strong style='color: #9FAECB;'>" +
        summary["attention"][0] +
        "</strong> and <strong style='color: #9FAECB;'>" +
        summary["attention"][1] +
        "</strong>. <br>";
      ...
      summary_text +=
        "There is also a clear opportunity to <strong>promote</strong> the following brands: <strong style='color: #B789BE;'>" +
        summary["brand"][0] +
        "</strong>, <strong style='color: #B789BE;'>" +
        summary["brand"][1] +
        "</strong>, and <strong style='color: #B789BE;'>" +
        summary["brand"][2] +
        "</strong> " +
        " All of these brands have significant affinity with the fans you are looking to attract.<br>";

      if (summary["concepts"].length == 2) {
        summary_text +=
          "From a <strong>creative</strong> point of view within the advertising, you will have a greater chance of appealing to this audience if you focus on the themes of <strong style='color: #4A946C;'>" +
          summary["concepts"][0].replace("horror", "conflict") +
          "</strong>, and <strong style='color: #4A946C;'>" +
          summary["concepts"][1].replace("horror", "conflict") +
          "</strong>.<br>";
      } else {
        summary_text +=
          "From a <strong>creative</strong> point of view within the advertising, you will have a greater chance of appealing to this audience if you focus on the theme of " +
          summary["concepts"][0] +
          ".<br>";
      }

      summary_text +=
        "<strong>Explore</strong> the tabs below to better understand the data behind the above recommendations and the <strong>why</strong> behind the <strong>what</strong>.";

      $(".movie-summary").html(summary_text);
      //}
    });
  ...
  fetch(URL)
    ...
    .then(data => {
      ...
      for (let attitude of fan_top) {
        const att =
          '<li><div class="favorable-list-item">' +
          attitude["a"] +
          '</div><div class="figure">' +
          Math.round(attitude["v"] * 100) +
          "</div></li>";
        $("#fan-favorable").append(att);
      }
      for (let attitude of fan_bot) {
        const att =
          '<li><div class="unfavorable favorable-list-item">' +
          attitude["a"] +
          '</div><div class="figure">' +
          Math.round(attitude["v"] * 100) +
          "</div></li>";
      ...
      for (let concept of data["concepts"]) {
        ...
        const conc =
          '<li class="conceptual-links-item"><div class="concept-hed">' +
          Object.keys(concept)[0].replace('horror', 'conflict') +
          '</div><div class="concept-icon">' +
          fillConcepts(Object.keys(concept)[0]) +
          '</div><div class="concept-words">' +
          concept[Object.keys(concept)]["words"].join(", ") +
          "</div></li>";
...
var getBrandData = function(movie) {
  ...
  for (b in brand_list) {
    URL =
      ...
      brand_list[b];
    let targeter =
      "#" + brand_list[b].replace(" ", "-") + " > .brand-list-item";
...
var getDemoData = function(movie) {
  ...
  fetch(URL)
    ...
    .then(data => {
      dotPlot(data["chart_data"], "#demo-income");
    })
    .catch(e => {
      console.log(e);
      $(".page-container").removeClass("clicked");
  ...
  fetch(URL)
    ...
    .then(data => {
      dotPlot(data["chart_data"], "#demo-gender");
    })
    .catch(e => {
      console.log(e);
      $(".page-container").removeClass("clicked");
...
var fillConcepts = function(concept) {
  ...
  switch (concept) {
    ...
    case "performing":
      ...
      break;
    case "monsters":
      icon = '<i class="fas fa-teeth-open"></i>';
      break;
    case "drugs":
